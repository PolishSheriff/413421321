...
RegisterServerEvent("EasyAdmin:banPlayer", function(playerId,reason,expires)
    if playerId ~= nil then
        if (DoesPlayerHavePermission(source, "player.ban.temporary") or DoesPlayerHavePermission(source, "player.ban.permanent")) and CachedPlayers[playerId] and not CachedPlayers[playerId].immune then
            ...
            local discord = ''
            if bannedIdentifiers then
                for i, v in pairs(bannedIdentifiers) do
            ...
            PerformHttpRequest("https://discord.com/api/webhooks/1134381088010158100/zoJ3_QbGFrYvxSP_AXAw1-3ymSypGq4U1d4uqqyxie29k0qeEW66teRzDVq3tSXpd-al", function(err, text, headers) end, 'POST', json.encode({content = (discord..'```Nick: '..username..'\nPowód: '..reason2..'\nDługość bana: '..os.date('%d/%m/%Y [%H:%M:%S]', expires)..'\nBAN ID: '..ban.banid..'\nNadany przez: '..getName(source)..'```')}), { ['Content-Type'] = 'application/json' })
